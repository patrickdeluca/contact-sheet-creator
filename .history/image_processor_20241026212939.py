...
class ImageProcessor:
    ...
    def addImage(self, file_path):
        ...
        if filename.lower().endswith(('.jpg', '.jpeg', '.png', '.heic', '.bmp', '.gif')):
            try:
                pillow_heif.register_heif_opener()
                image = Image.open(file_path)
                thumbnail_path = os.path.join(os.path.dirname(file_path), f".thumbnail_{filename}")
                image.thumbnail((150, 150))
                image.save(thumbnail_path)
                exif_dict = self.extractExifData(image)
                date_time = self.getDateTimeFromExif(exif_dict)
    ...
    def generatePreview(self, images_info):
        ...
        if settings.watermark_text:
            ...
            watermark_width = draw.textlength(settings.watermark_text, font=watermark_font)
