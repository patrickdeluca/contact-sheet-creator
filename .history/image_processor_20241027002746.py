...
class ImageProcessor:
    ...
    def _calculate_layout(
        ...
        max_rows = (usable_height - margin) // (cell_min_size + margin + 50)  # +50 for caption
    ...
    def _generate_pages(
        ...
        for i in range(0, len(images_info), images_per_page):
            ...
            total_pages = (len(images_info) + images_per_page - 1) // images_per_page
            page_text = f"Page {page_num} of {total_pages}"
            draw.text(
                (page_size[0] - margin - draw.textlength(page_text, font=font), y_offset),
                page_text,
                fill='black',
                font=font
            )
            ...
            if self.settings_manager.watermark_text:
                self._add_watermark(page, draw, page_size[0], page_size[1], margin)
    ...
    def _add_watermark(
        ...
        text_height = 40  # Use actual font size instead of settings
        position = (
    ...
    def generate_preview(self, images_info: List[Dict]) -> Optional[Image.Image]:
        ...
        try:
            ...
            preview_width = 800
            preview_height = int(preview_width * (11/8.5))  # Maintain aspect ratio
