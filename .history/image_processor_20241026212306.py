...
class ImageProcessor:
    ...
    def createContactSheet(self, images_info):
            ...
            # Determine images per page to maximize image size (3-4 images per page)
            images_per_page_options = [3, 4]

            for images_per_page in images_per_page_options:
                # Calculate thumbnail sizes
                cols = 1 if images_per_page == 3 else 2
                rows = (images_per_page + cols - 1) // cols
                thumb_width = (page_width - (cols + 1) * margin) // cols
                thumb_height = (page_height - (rows + 1) * margin - 200) // rows  # Adjusted for context

                if thumb_width > 0 and thumb_height > 0:
                    break  # Found suitable configuration
            ...
            for i in range(0, total_images, images_per_page):
                ...
                if context_text:
                    draw.text((margin, y_offset), context_text, fill='black', font=font)
                    y_offset += font_size + margin  # Adjust based on text height

                # Adjust thumb_height if space remains
                available_height = page_height - y_offset - margin
                rows = (min(images_per_page, total_images - i) + cols - 1) // cols
                thumb_height = (available_height - (rows - 1) * (margin + 50)) // rows

                for idx, info in enumerate(images_info[i:i + images_per_page]):
                    row = (idx // cols)
                    col = (idx % cols)
                    x = margin + col * (thumb_width + margin)
                    y = y_offset + row * (thumb_height + margin + 50)  # 50 for text under image

                    img = Image.open(info['path'])
                    img_resized = img.copy()
                    img_resized.thumbnail((thumb_width, thumb_height))
                    ...
                    if watermark_text:
                        ...
                        watermark_position = (page_width - watermark_width - margin, page_height - watermark_height - margin)
                        draw.text(watermark_position, watermark_text, fill='grey', font=watermark_font)
            ...
            for idx, page in enumerate(pages):
                ...
                page_path = os.path.join(save_folder, f"{filename}.{export_format}")
                if export_format == 'pdf':
                    page.save(page_path, 'PDF', resolution=300)
                elif export_format in ['jpg', 'jpeg']:
                    page.save(page_path, 'JPEG', quality=quality)
                elif export_format == 'png':
                    page.save(page_path, 'PNG', compress_level=int((100 - quality) / 10))
                else:
                    page.save(page_path)
            return True

        def generatePreview(self, images_info):
            # Similar to createContactSheet but generate a single page for preview
            if not images_info:
                return None

            settings = self.settings_manager
            page_width, page_height = 800, 600  # Smaller size for preview
            margin = 10
            ...
            rows = (images_per_page + cols - 1) // cols
            thumb_width = (page_width - (cols + 1) * margin) // cols
            thumb_height = (page_height - (rows + 1) * margin - 50) // rows  # Adjusted for context
            ...
            if settings.context_text:
                draw.text((margin, y_offset), settings.context_text, fill='black', font=font)
                y_offset += settings.font_size + margin  # Adjust based on text height

            for idx, info in enumerate(images_info[:images_per_page]):
                row = (idx // cols)
                col = (idx % cols)
                x = margin + col * (thumb_width + margin)
                y = y_offset + row * (thumb_height + margin + 30)  # 30 for text under image
                ...
                if settings.watermark_text:
                    ...
                    watermark_width, watermark_height = draw.textsize(settings.watermark_text, font=watermark_font)
                    watermark_position = (page_width - watermark_width - margin, page_height - watermark_height - margin)
                    draw.text(watermark_position, settings.watermark_text, fill='grey', font=watermark_font)
